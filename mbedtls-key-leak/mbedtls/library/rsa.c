                 ...
                 unsigned char *output )
{
    ...
    // TODO(dwe): here we could disable the CRT decryption (currently disabled by me in config.h)
#if defined(POLARSSL_RSA_NO_CRT)
    //printf("mpi_exp_mod caller (no-CRT)\n");
    MPI_CHK( mpi_exp_mod( &T, &T, &ctx->D, &ctx->N, &ctx->RN ) );
#else
    //printf("mpi_exp_mod caller (CRT)\n");
    /*
